defmodule ZeroCoupledWeb.CartLive.CheckoutView do
  ...
  defp steps(assigns) do
    ...
    <ol class="flex gap-4 text-sm mb-6">
      <li :for={{s, label} <- Generated.flow_milestones(:checkout)} class={[
        "font-medium",
  ...
  defp step_body(%{step: :address} = assigns) do
    ~H"""
    <.simple_form for={@address_form} phx-change={Contracts.event(:validate_address)} phx-submit={Contracts.event(:submit_address)}>
      <.input field={@address_form[:name]} label="Full name" phx-hook={Contracts.hook(:auto_focus)} id="address_name" />
  ...
  defp step_body(%{step: :payment} = assigns) do
    ...
    <div class="space-y-4">
      ...
      <div class="flex gap-3">
        <button phx-click={Contracts.event(:edit_address)} class="text-sm text-zinc-500 underline">Edit address</button>
        <button phx-click={Contracts.event(:pay)} class="rounded-lg bg-zinc-900 py-2 px-4 text-sm font-semibold text-white">
          Pay <%= @cart.total %>
        </button>
      </div>
    </div>
