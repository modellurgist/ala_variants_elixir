...
defmodule ZeroCoupledWeb.CartPage.Session do
  ...
  def new(opts \\ []) do
    struct!(__MODULE__,
      cart:
        ZeroCoupled.Features.CartItems.init(
          Keyword.put(opts, :config,
            pricing: [
              ...
              promo: %{"HALF" => 50, "SAVE10" => 10, "SAVE20" => 20},
              gift_wrap_unit: 299
            ]
          )
        ),
...
defmodule ZeroCoupledWeb.CartPage.Generated do
  ...
  def apply_fact(session, %ZeroCoupled.Foundation.Broadcast.Facts.StockChanged{} = fact) do
    ...
    (
      ...
      {new_slot, more} =
        ZeroCoupled.Features.CartItems.Intents.set_stock(Map.fetch!(session, :cart), payload)
  ...
  (
    ...
    def put_session(socket, session) do
      ...
      stale_saved = render_stale?(socket, :saved_slot, Map.fetch!(session, :saved))
      stale_wishlist = render_stale?(socket, :wishlist_slot, Map.fetch!(session, :wishlist))
      ...
      socket =
        if stale_saved do
          assign(
            socket,
            :saved,
            ZeroCoupled.Features.SavedItems.render_data(Map.fetch!(session, :saved))
          )
        else
          socket
        end
      ...
      socket =
        if stale_wishlist do
          assign(
            socket,
            :wishlist,
            ZeroCoupled.Features.Wishlist.render_data(Map.fetch!(session, :wishlist))
          )
        else
          socket
        end
  ...
  (
    ...
    def flow_milestones(:checkout) do
      [address: "Address", payment: "Payment"]
    end
  )

  def flow_path(:checkout, :address) do
    "/cart/checkout"
  end

  def flow_path(:checkout, :payment) do
  ...
  (
    ...
    def flow_param_step(:checkout, params) do
      case params["step"] do
        nil -> :address
        step -> Map.get(%{"address" => :address, "payment" => :payment}, step)
      end
    end
  )
  ...
  (
    ...
    def __manifest__ do
      %{
        ...
        features: [
          {:cart, ZeroCoupled.Features.CartItems,
           render: :render_data,
           config: [
             pricing: [
               shipping: %{
          ...
          {:saved, ZeroCoupled.Features.SavedItems, render: :render_data},
          {:wishlist, ZeroCoupled.Features.Wishlist, render: :render_data},
        ...
        reactions: [
          ...
          {ZeroCoupled.Foundation.Broadcast.Facts.StockChanged,
           [{:cart, ZeroCoupled.Features.CartItems.Intents, :set_stock, transform: false}]}
        ]
      }
    end
  )
