...
defmodule ZeroCoupled.Features.CheckoutFlow do
  ...
  @spec init(keyword()) :: t()
  def init(_opts), do: %__MODULE__{step: ZeroCoupled.Flows.initial(:checkout)}
  ...
  @doc """
  Validate and store the address; the step advance comes from the flow
  table (V33), not a hardcoded next step.
  """
  ...
  def submit_address(%__MODULE__{} = checkout, params) do
    ...
    case Ecto.Changeset.apply_action(changeset, :insert) do
      {:ok, address} ->
        {:ok, goto(%{checkout | address: address}, advance(checkout, :submit_address))}
  ...
  def advance(%__MODULE__{step: step}, event), do: ZeroCoupled.Flows.advance(:checkout, step, event)

  @spec goto(t(), atom()) :: t()
  def goto(%__MODULE__{} = checkout, :no_transition), do: checkout
  def goto(%__MODULE__{} = checkout, step), do: %{checkout | step: step}

  @doc """
  URL-navigation policy: the browser (back button / typed URL) may move the
  wizard only along the `:edit_address` edge — backwards to the address
  step. Forward jumps would skip validation and are ignored. The flows
  table declares the edges; which of them are URL-jumpable is feature
  policy.
  """
  @spec can_goto?(t(), atom()) :: boolean()
  def can_goto?(%__MODULE__{} = checkout, step), do: advance(checkout, :edit_address) == step

  # Result-fact edges (async payment settled) — deliberately not in the
  # flows table; see the manifest's flows/0 doc.
  ...
  @spec reset(t()) :: t()
  def reset(%__MODULE__{}), do: %__MODULE__{step: ZeroCoupled.Flows.initial(:checkout)}
end
...
defmodule ZeroCoupled.Features.CheckoutFlow.Intents do
  ...
  def start_checkout(session, _args) do
    if Cart.empty?(session.cart) do
      ...
      {put_slot(session, CheckoutFlow.reset(session.checkout)),
       [Effects.patch_flow(:checkout, ZeroCoupled.Flows.initial(:checkout))]}
    end
  end
  ...
  def edit_address(session, _args) do
    step = CheckoutFlow.advance(session.checkout, :edit_address)

    {put_slot(session, CheckoutFlow.goto(session.checkout, step)),
     [Effects.patch_flow(:checkout, step)]}
  end

  @doc """
  Irregular (from handle_params): the URL asked for a step — browser back/
  forward or a typed address. Applied only when the feature's URL policy
  allows it; otherwise the session stays authoritative.
  """
  def goto_step(session, step) do
    if CheckoutFlow.can_goto?(session.checkout, step),
      do: {put_slot(session, CheckoutFlow.goto(session.checkout, step)), []},
      else: {session, []}
  end
  ...
  def submit_address(session, params) do
    case CheckoutFlow.submit_address(session.checkout, params) do
      {:ok, checkout} ->
        {:ok, put_slot(session, checkout), [Effects.patch_flow(:checkout, checkout.step)]}
  ...
  def pay(session, stock_levels) do
    with :ok <- Cart.check_ready(session.cart, stock_levels),
         step when step != :no_transition <- CheckoutFlow.advance(session.checkout, :pay) do
      {put_slot(session, CheckoutFlow.goto(session.checkout, step)),
       [
      ...
      # No :pay edge from the current step (declared in the flow table).
      :no_transition ->
        {session, []}
    end
  end
