...
> This directory is the **V35** end state: V30's committed-codegen core (documented in full below,
> unchanged) plus four cumulative additions — **V32** presentation purity, **V33** the flows
> channel, **V34** calibration-on-the-diagram, **V35** composed cross-slot inputs. The V30 sections
> below still describe the spine accurately; the section immediately following covers what V32→V35
> added. How-to: [`techniques.md`](techniques.md) (one recipe per mechanism) and
> [`adding-a-feature.md`](adding-a-feature.md) (a verified end-to-end walkthrough).

## What V32→V35 added (on top of the V30 spine)

New layers and channels, all pointing *down*:

- **`Web.Contracts`** (V32) — the single home for every event/stream/hook/push-event string. Markup
  and emitters reference it; `assets/js/contracts.js` is generated from it (`mix zc.gen.contracts`).
- **`Catalog.*`** (V32) — generic, zero-coupled presentation abstractions (`ProductLineRow`,
  `ActionButton`, `StockBadge`) that render **projected port values**, never domain structs. Each
  feature's `Components` compose these; the projection happens in the intent (the `row/…` port).
- **Three purity checkers** (V32) — `CorePurity` (core ↛ web, inherited), plus `ComponentPurity`
  (presentation ↛ domain) and `ContractPurity` (no restated contract literals, app-wide). Run by
  `credo --only Purity` in the `test` alias; a coupling regression fails the build.
- **Typed cross-process facts** (V32) — `Broadcast.Facts.*` structs replace tagged tuples; the
  manifest routes them like any in-process fact via the page's generic `handle_info`.
- **`flows/0` + `ZeroCoupled.Flows`** (V33) — wizard skeletons (steps, transitions, milestones,
  per-step URLs) declared in the manifest; the transition table generates into a core module,
  URL/milestone glue into the page. `Effects.patch_flow` keeps URLs out of the pure core.
- **The `config:` channel** (V34) — domain abstractions are generic; calibration (rates, promo
  codes, tiers, the undo window) lives in the manifest and is spliced into `init/1`. Reading a
  manifest is reading that consumer's policy.
- **Cross-slot inputs resolved in the composition** (V35) — a feature reads/writes only its own
  slot; the shell resolves peer-slot data and passes it in. A `CrossSlotRead` detector reports
  violations; `run_reaction_audit.exs` checks reaction placement (manifest vs shell).

The **second consumer** (`PortalPage`, a B2B bulk-order portal) shares all of the above — the same
`Cart` aggregate, domain abstractions, `Undo` feature, contracts, catalog, generator, and flows
machinery — as the D6 reuse experiment. Its author surface measured **0.58× the first consumer's**.

Everything below this line is the V30 spine, still accurate.

---

# V30 Architecture (the spine)
