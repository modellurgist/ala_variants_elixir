defmodule ZeroCoupled.Cart do
  ...
  @doc "Build a cart from persisted items."
  @spec new(keyword()) :: t()
  def new(opts \\ []) do
    %__MODULE__{
  ...
  def apply_promo(%__MODULE__{} = cart, code) do
    case ValidatePromo.call(code) do
      {:ok, pct} ->
  ...
  defp recalc(%__MODULE__{} = cart) do
    ...
    gift_wrap = CalculateGiftWrapCost.call(MapSet.size(cart.gift_wrapped_ids))
    shipping = CalculateShipping.call(cart.shipping_method, subtotal)
