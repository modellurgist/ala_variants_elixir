defmodule ZeroCoupledWeb.CartPageTest do
  ...
  describe "wishlist" do
    test "toggle_wishlist adds the item's product" do
      {s, effects} = CartPage.run_intent(session(), :toggle_wishlist, %{item_id: 1})
    ...
    test "toggle_wishlist for an unknown item is a no-op" do
      assert {_s, []} = CartPage.run_intent(session(), :toggle_wishlist, %{item_id: 999})
    end

    test "remove_wishlist streams a delete" do
      {s, _} = CartPage.run_intent(session(), :toggle_wishlist, %{item_id: 1})
  ...
  describe "real-time stock and tabs" do
    test "set_stock updates the cart item" do
      {s, effects} = CartPage.run_pure(session(), &CartItems.Intents.set_stock(&1, 10, 2))
      assert Enum.find(s.cart.items, &(&1.product.id == 10)).product.stock == 2
      assert has?(effects, &match?(%Effects.StreamInsert{name: :cart_items}, &1))
    end

    test "set_stock for an absent product is a no-op" do
      assert {_s, []} = CartPage.run_pure(session(), &CartItems.Intents.set_stock(&1, 999, 0))
    end
  ...
  describe "checkout flow" do
    ...
    test "start_checkout patches to the checkout route" do
      {_s, effects} = CartPage.run_intent(session(), :start_checkout, %{})
      assert has?(effects, &match?(%Effects.Patch{to: "/cart/checkout"}, &1))
    end
    ...
    test "submit_address advances to payment; edit_address goes back" do
      ...
      assert has?(effects, &match?(%Effects.Patch{to: "/cart/checkout/payment"}, &1))
    ...
    test "pay validates stock and starts async checkout" do
      {s, effects} = CartPage.run_pure(session(), &CheckoutFlow.Intents.pay(&1, %{10 => 10, 20 => 10}))
